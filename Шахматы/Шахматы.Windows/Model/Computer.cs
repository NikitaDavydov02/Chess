...
namespace Шахматы.Model
{
    class Computer
    {
        ...
        //Methods;
        public void StartTheGame(int difficalty)
        {
            Difficalty = difficalty;
            IndexOfMover = 1;
            virtualBoard = new VirtualBoard();
            virtualBoard.StartTheGame();
            ToArrangeThePieces();
        }
        ...
        {
            ShowMayMoves(startX, startY);
            virtualBoard.startX = startX;
            virtualBoard.startY = startY;
            virtualBoard.MakeMove(endX, endY);
        ...
        public void ToThinkAboutMove(out int endX, out int endY)
        {
            Dictionary<int, double> ratingOfMoves = new Dictionary<int, double>();
            virtualBoard.ShowMayMoves();
            foreach (int i in virtualBoard.allMayMoves)
                ratingOfMoves.Add(i, 0);
            List<int> keys = ratingOfMoves.Keys.ToList();
            for (int i=0; i < ratingOfMoves.Count; i++)
            {
                int move = keys[i];
                int sX;
                int sY;
                ...
                move = move - eX * 10;
                eY = move;
                ratingOfMoves[keys[i]] = ToAnalizeTheProgress(sX, sY, eX, eY,Difficalty, 1);
            }
            double maxRating = ratingOfMoves.Values.Max();
            for (int i = 0; i < ratingOfMoves.Count; i++)
            {
                if (ratingOfMoves[keys[i]] == maxRating)
                {
                    int move = keys[i];
                    startX = move / 1000;
                    move = move - startX * 1000;
                    startY = move / 100;
                    move = move - startY * 100;
                    endX = move / 10;
                    move = move - endX * 10;
                    endY = move;
                    return;
        ...
        private double ToAnalizeTheProgress(int sX, int sY, int eX, int eY, int maxDeep, int currentDeep)
        {
            virtualBoard.RememberPosition();
            virtualBoard.startX = sX;
            virtualBoard.startY = sY;
            virtualBoard.MakeMove(eX, eY);
            double theArithmeticMean = 0;
            if (virtualBoard.EndOfGame && virtualBoard.IndexOfMover == IndexOfHuman)
            {
                virtualBoard.ReturnPosition();
                return 1;
            }
            if (virtualBoard.EndOfGame && virtualBoard.IndexOfMover != IndexOfHuman)
            {
            ...
            if (!virtualBoard.EndOfGame && maxDeep == currentDeep)
            {
                virtualBoard.ReturnPosition();
                return 0;
            }
            List<double> raitingsOfMoves = new List<double>();
            virtualBoard.ShowMayMoves();
            List<int> mayMoves = new List<int>();
            foreach (int i in virtualBoard.allMayMoves)
                mayMoves.Add(i);
            for(int i = 0; i < mayMoves.Count(); i++)
            {
                int move = mayMoves[i];
                int secSX;
                int secSY;
                ...
                int secEY;
                secSX = move / 1000;
                move = move - secSX * 1000;
                secSY = move / 100;
                move = move - secSY * 100;
                secEX = move / 10;
                move = move - secEX * 10;
                secEY = move;
                double rating = ToAnalizeTheProgress(secSX, secSY, secEX, secEY, maxDeep, currentDeep + 1);
                raitingsOfMoves.Add(rating);
            }
            double sum = raitingsOfMoves.Sum();
            theArithmeticMean += sum / raitingsOfMoves.Count;
            virtualBoard.ReturnPosition();
